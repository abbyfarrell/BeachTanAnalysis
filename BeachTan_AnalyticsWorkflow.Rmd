...
There are a couple observations we can make from the graphs above. First, looking at the correlation plot of numeric variables we can see that upgrade revenue has the strongest correlation with the number of UV Tans (Sunless Tans second). The scatter plots showing the different factors against our target variable tells us which membership type and level contributed most to upgrade revenue. Now, we can take a closer look at these variables that seem to have the greatest correlation to confirm our findings.
