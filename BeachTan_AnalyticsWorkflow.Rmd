...
The goal of the following analysis is to aid the Marketing Team of Beach Tan Inc. in understanding factors that contribute to a customer's likelihood of spending money in upgrade revenue within the company. I am performing an analysis on the customer data provided by Beach Tan to explain any correlation to upgrade revenue found.

# 1 Loading & Cleaning Data

The beginning code chunk includes installing packages needed for the analysis, as well as any recoding done to ensure accurate results. The output below shows a summary of the variables after the data has been prepped, as well as a plot introducing the data.
...
kable(summary(df), format="pipe", digits=2)
plot_intro (df)
```

# 2 Correlation {.tabset .tabset-pills}
...
plot_correlation(na.omit(df), maxcat = FALSE)
```
